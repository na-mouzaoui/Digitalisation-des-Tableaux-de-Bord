...
export async function fetchKpiRowsMap(tabKeys: string[], domain?: string): Promise<Record<string, string[]>> {
  const uniqueKeys = Array.from(new Set(tabKeys.map((key) => key.trim()).filter(Boolean)))
  const entries = await Promise.all(
    uniqueKeys.map(async (key) => {
      try {
        const query = domain ? `?domain=${encodeURIComponent(domain)}` : ""
        const response = await authFetch(`/api/kpis/by-name/${encodeURIComponent(key)}${query}`, { cache: "no-store" })
