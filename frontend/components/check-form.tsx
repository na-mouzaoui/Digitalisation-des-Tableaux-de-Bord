...
export function CheckForm({ user }: CheckFormProps) {
  ...
  const [isRegionalUser, setIsRegionalUser] = useState(false)
  ...
  const loadBanks = useCallback(async () => {
    ...
    const normalizedBanks = await Promise.all(payload.map(async (bank: Bank & { positionsJson?: string }) => {
      // Try to get user-specific calibration
      const calibrationResponse = await requestWithAuth(`${API_BASE}/api/banks/${bank.id}/user-calibration`)
  ...
  useEffect(() => {
    ...
    const loadRegionData = async () => {
      if (user.role === "regionale" && user.region) {
        ...
        setIsRegionalUser(false)
        setCity("Alger") // Default for non-regional users
      }
    }
    loadRegionData()
  }, [])
  ...
  const checkReferenceUnique = async (ref: string): Promise<{ unique: boolean; error?: string }> => {
    try {
      ...
      if (response.ok) {
        const data = await response.json()
        return { unique: !data.exists, error: data.exists ? `La référence ${ref} est déjé utilisée` : undefined }
      }
      return { unique: true }
    } catch (error) {
      return { unique: true }
  ...
  const validateReference = (ref: string, checkbook: Checkbook | undefined): { valid: boolean; error?: string } => {
    ...
    // La référence doit étre au format: S??RIE (2 car.) + NUM??RO (7 chiffres)
    if (ref.length !== 9) {
      return { valid: false, error: "La référence doit contenir exactement 9 caractéres (2 lettres + 7 chiffres)" }
    }
    ...
    if (isNaN(numero)) {
      return { valid: false, error: "Les 7 derniers caractéres doivent étre des chiffres" }
    }
  ...
  const handlePrintConfirmed = async () => {
    ...
    try {
      ...
      // Sauvegarder le chéque dans la base de données
      await createCheck({
      ...
      toast({
        title: "Succes",
        description: "Chéque enregistré et PDF généré",
      })

      // Réinitialiser le formulaire aprés un délai
      setTimeout(() => {
        setAmount("")
        setAmountInWords("")
        setPayee("")
        // Reset to appropriate default city based on user role
        if (isRegionalUser && regionCities.length > 0) {
      ...
      toast({
        title: "Erreur",
        description: error instanceof Error ? error.message : "??chec de l'impression",
        variant: "destructive",
      })
      setIsPrinting(false)
    }
        ...
        <Card>
          <CardContent className="pt-6">
            <form className="space-y-4" onSubmit={(e) => e.preventDefault()}>
              ...
              <div className="space-y-2">
                <Label htmlFor="payee">?? l'ordre de</Label>
                <Select 
                  value={payee} 
              ...
              <div className="grid gap-4 sm:grid-cols-2">
                <div className="space-y-2">
                  ...
                  <Select value={city} onValueChange={setCity} required disabled={!isBankSelected}>
                    ...
                    <SelectContent>
                      {VILLES.map((wilaya) => {
                        const isCityAllowed = !isRegionalUser || regionCities.includes(wilaya.name)
                        return (
                          <SelectItem 
                            key={wilaya.code} 
                            value={wilaya.name}
              ...
              <div className="space-y-2">
                <Label htmlFor="reference">Référence du chéque</Label>
                <Input
                  id="reference"
                  type="text"
                ...
                <Button
                  ...
                  onClick={() => {
                    ...
                    if (numericAmount === null || numericAmount <= 0) {
                      toast({
                        title: "Erreur",
                        description: "Le montant doit étre un nombre valide et positif",
                        variant: "destructive"
                      })
                      return
                    }
